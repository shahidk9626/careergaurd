                ...
                <div class="p-6 pb-0 mb-0 bg-white border-b-0 rounded-t-2xl">
                    <div class="flex flex-wrap -mx-3">
                        <div class="flex items-center flex-none w-1/2 max-w-full px-3">
                            <h6 class="mb-0">User Permissions Overrides</h6>
                        </div>
                    </div>
                </div>
                <div class="flex-auto px-0 pt-0 pb-2">
                    <div class="p-6 overflow-x-auto">
                        <table id="userPermTable" class="items-center w-full mb-0 align-top border-gray-200 text-slate-500">
        ...
        $(document).ready(function () {
            $('#userPermTable').DataTable({
                ...
                columns: [
                    {
                        data: 'id',
                        className: 'text-sm px-6 border-b'
                    },
                    {
                        data: 'name',
                    ...
                    },
                    {
                        ...
                        render: function (data) {
                            ...
                            return `
                                <span class="text-xxs px-2.5 py-1.4 inline-block whitespace-nowrap text-center align-baseline font-bold uppercase leading-none text-white rounded-1.8 ${badgeClass}">${data}</span>
                            `;
                        }
                    },
                    {
                        ...
                        render: function (data) {
                            if (data.id === 1) {
                                return `<span class="text-xs font-bold text-green-500">All Access Granted</span>`;
                            }

                            // Find staff slug if available
                            let editUrl = '#';
                            // Assuming you have access to a way to get staff slug, or use ID if synced.
                            // For now, I'll use a generic link or rely on the user having a staff record.

                            return `
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ url('user-permissions') }}/${data.id}" class="inline-block px-4 py-2 mb-0 text-xs font-bold text-center text-white uppercase transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro ease-soft-in bg-150 tracking-tight-soft bg-x-25 bg-gradient-to-tl from-purple-700 to-pink-500 hover:scale-102 hover:shadow-soft-xs active:opacity-85">
                                        <i class="fas fa-user-lock mr-1"></i> Manage Permissions
                                    </a>
                                    <a href="{{ url('staff') }}" class="inline-block px-4 py-2 mb-0 text-xs font-bold text-center text-slate-700 uppercase transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro ease-soft-in bg-150 tracking-tight-soft bg-gray-100 hover:scale-102 hover:shadow-soft-xs active:opacity-85">
