        ...
        <ul class="flex flex-col pl-0 mb-0">
            ...
            @if(auth()->user()->role_id !== 0)
                @if(hasPermission('activity-logs.view'))
                    @php $isActivityActive = request()->routeIs('admin.activity-logs.*') || request()->is('*activity-logs*'); @endphp
                    <li class="w-full mt-0.5">
                        <a class="py-2.7 text-sm ease-nav-brand my-0 mx-4 flex items-center whitespace-nowrap px-4 transition-colors rounded-lg {{ $isActivityActive ? 'bg-white shadow-soft-xl font-semibold text-slate-700' : 'text-slate-700 hover:bg-gray-50' }}"
                            href="{{ route('admin.activity-logs.index') }}">
                            <div
                                class="mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-center stroke-0 text-center xl:p-2.5 {{ $isActivityActive ? 'bg-gradient-to-tl from-purple-700 to-pink-500 shadow-soft-2xl' : 'bg-white shadow-soft-2xl' }}">
                                <i class="fas fa-history {{ $isActivityActive ? 'text-white' : 'text-slate-700' }}"></i>
                            </div>
