...
<body class="m-0 font-sans antialiased font-normal text-base leading-default bg-gray-50 text-slate-500">
    ...
    @if(auth()->check() && auth()->user()->role_id === 0)
        ...
        <script>
            $(document).ready(function() {
                const userProfileCompleted = {{ auth()->user()->profile_completed }};
                const userVerificationStatus = "{{ auth()->user()->verification_status }}";
                
                // Do not show modal on profile editing or registration wizard where they are completing it
                const isRegistrationPage = {{ request()->routeIs('customer.registration') ? 'true' : 'false' }};
                const isProfilePage = {{ request()->routeIs('customer.profile') ? 'true' : 'false' }};
                const isEditProfilePage = {{ request()->routeIs('customer.profile.edit') ? 'true' : 'false' }};
                ...
                function showReminderModal() {
                    ...
                    if (userProfileCompleted === 1 && userVerificationStatus !== 'verified') {
                        ...
                        primaryBtn.href = "{{ route('customer.profile') }}";
                        ...
                        primaryBtn.href = "{{ route('customer.registration') }}";
                        primaryBtn.style.display = 'block';
